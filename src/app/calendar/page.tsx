...
export default async function CalendarPage() {
  const days = await Promise.all(
    Array.from({ length: 7 }, async (_, index) => {
      ...
      const rankings = await getRankingsForDate(date, { avoidOffshoreWind: true });
  ...
  return (
    <main className="mx-auto max-w-7xl px-4 py-8 sm:px-6 lg:px-8">
      <div className="mb-6">
        <p className="text-sm font-black uppercase tracking-normal text-lagoon">Forecast calendar</p>
        <h1 className="mt-1 text-3xl font-black text-slate-950">Seven-day kite window scan</h1>
      </div>
      <section className="grid gap-4 md:grid-cols-2 xl:grid-cols-4">
        {days.map((day) => (
          <article key={day.date.toISOString()} className="rounded-lg border border-slate-200 bg-white p-4 shadow-soft">
            <h2 className="text-lg font-black text-slate-950">{format(day.date, "EEE dd MMM")}</h2>
